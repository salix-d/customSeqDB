...
# NCBI database
Functions specific to this db are here : https://github.com/salix-d/customSeqDB/blob/master/R/makingDB_ncbiFuns.R
## Getting the sequences details
This gets the ESummary of all records in the NCBI nucleotide database matching these taxa and gene which don't have 'UNVERIFIED' in the keyword fields.
It then parse the data and returns a list with history server info in case we want to look at the records again and a list of details including the accession numbers, the taxonomic ids and the description. The ESearch can return up to 100k results. This part may take a while.

ncbi includes 6 orders and the COI gene can be written 4 ways.
```{r}
ncbi <- get_ncbiDetails(c("Opilioacarida", "Holothyrida", "Ixodida", "Mesostigmata", "Trombidiformes", "Sarcoptiformes"), c("COI", "CO1", "COXI", "COX1"), customTerm = "NOT UNVERIFIED[KYWD]")
```
...
write.fasta(sequences = ncbi$complete$COI$seq, ids = ncbi$complete$tax$id, outFile = "ncbiCompleteCOI_ncbi.fasta")
```
...
# ENA database
Functions specific to this db are here : https://github.com/salix-d/customSeqDB/blob/master/R/makingDB_enaFuns.R
## Getting the taxIds
...
write.fasta(sequences = ena$complete$COI$seq, ids = ena$complete$tax$id, outFile = "enaCompleteCOI_ena.fasta")
```
...
write.fasta(sequences = ncbi_ena$sequence, ids = ncbi_ena.tax$id, outFile = "completeCOI_ncbi_ena.fasta")
```
